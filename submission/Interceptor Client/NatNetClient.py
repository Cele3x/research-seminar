...
class NatNetClient:
    def __init__(self):
        ...
        # Model Array
        self.modelNames = ["Backwards12F","Forwards12F","Forwards24F","Forwards36F","Forwards48F"]
        self.models = {}

        # Negative Model
        self.queueFramesNegModel = []
        self.queueFramesNumberToDelay = 12
    ...
    def __processMessage(self, data):
        ...
        if (messageID == self.NAT_FRAMEOFDATA):
            ...
            # Intercept Negative Model
            if(self.interceptionStatus == True and self.activePredictionModelIndex == 0):

                # Queue is Full
                if(len(self.queueFramesNegModel) == self.queueFramesNumberToDelay):
                    self.redirSocket.sendto(self.queueFramesNegModel.pop(0), (self.outgoingTargetIpAddress, self.outgoingTargetPort))
                    self.queueFramesNegModel.append(data)
