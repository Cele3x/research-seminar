...
dfLogs2 <- read.csv("logsCombined.csv", header = TRUE, sep= ",")
...
dfLogsResultSet$condition[dfLogsResultSet$condition==1] <- 0
dfLogsResultSet$condition[dfLogsResultSet$condition==2] <- 12
dfLogsResultSet$condition[dfLogsResultSet$condition==3] <- 24
dfLogsResultSet$condition[dfLogsResultSet$condition==4] <- 36
dfLogsResultSet$condition[dfLogsResultSet$condition==5] <- 48
dfLogsResultSet$condition[dfLogsResultSet$condition==6] <- -12
...
round_df(anova(art(freq ~ condition * as.factor(seq) + (1|probandId), data=dfLogsResultSet.ext)))
round_df(anova(art(freq ~ condition + (1|probandId), data=dfLogsResultSet.ext)))

dfLogsResultSet.ext$condition <- as.numeric(dfLogsResultSet.ext$condition)

summary(lm(freq ~ condition +  (1|probandId), dfLogsResultSet.ext))
```
